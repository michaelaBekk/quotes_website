...
function querySQL(query)  {
      mySqlDb.query(query, (result, err) => {
            if(err) throw err;
            return result;
      })
}
...
module.exports = (app) => {
      ...
      app.post('/', [
            ...
            if(!errors.isEmpty()) {
              ...
              const subscriber = querySQL(`SELECT emails FROM subscribers WHERE emails='${req.body.email}'`);
                if(subscriber.length > 0) {
                  res.render('submit', {email_exists: true});
                }else {
                  let randomNumber = Math.floor(Math.random() * 15) + 9;
                  let user_ID = makeID(randomNumber);
                  querySQL(`INSERT INTO subscribers VALUES('${req.body.email}', '${user_ID}')`);
